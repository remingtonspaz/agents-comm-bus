...
export function formatDaemonStatus(snapshot) {
    const lines = ["agents-comm-bus status", ""];
    if (snapshot.daemon.reachable) {
        lines.push(`daemon: up (pid ${snapshot.daemon.pid ?? "?"}, port ${snapshot.daemon.port ?? "?"}, ` +
            `version ${snapshot.daemon.version ?? "?"}, protocol ${snapshot.daemon.protocol_version ?? "?"})`);
        if (snapshot.runtime) {
            lines.push(`runtime: pendingInbound=${snapshot.runtime.pending_inbound_depth}, ` +
    ...
    for (const row of snapshot.conversations.slice(0, 10)) {
        lines.push(`  ${row.agent}/${row.comm} chat=${row.chat_native_id} ` +
...
async function listWatcherPids(stateRoot) {
...
export function leasePathFor(comm, resourceId, homeDir = os.homedir()) {
