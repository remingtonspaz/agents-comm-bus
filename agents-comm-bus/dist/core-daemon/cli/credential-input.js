...
export async function resolveCredentialInput(options) {
    const sources = [];
    if (options.botToken)
        sources.push("--bot-token");
    if (options.credentials)
        sources.push("credentials");
    if (options.credentialsFile)
        sources.push("--credentials-file");
    if (options.credentialsJson)
        sources.push("--credentials-json");
    if (sources.length === 0) {
        throw new Error("credentials are required; pass --bot-token, --credentials-file <path.json>, " +
            "or --credentials-json <json>");
    }
    if (sources.length > 1) {
        throw new Error(`credential input is ambiguous; pass only one of ${sources.join(", ")}`);
    }
    let credentials;
    switch (sources[0]) {
        case "--bot-token":
...
//# sourceMappingURL=credential-input.js.map