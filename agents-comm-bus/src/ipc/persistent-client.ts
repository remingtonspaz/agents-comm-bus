...
} from "../bootstrap/ensure-daemon.js";
import {
  IPC_MESSAGE_TYPES,
  createClientHello,
  createRequest,
...
export class PersistentIpcClient {
  ...
  private hello: DaemonHello | null = null;
  private readonly replayRequests: ReplayRequest[] = [];
  ...
  constructor(private readonly options: PersistentIpcClientOptions) {}

  /** Connect for the first time and resolve once handshake completes. */
  async start(): Promise<DaemonHello> {
   ...
   */
  async registerReplay(method: string, params: unknown): Promise<unknown> {
  ...
  private async requestOnCurrentConnection(
    method: string,
    params: unknown,
  ): Promise<unknown> {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
  ...
  private handleMessage(data: RawData): void {
    ...
    if (message.type !== IPC_MESSAGE_TYPES.response) return;
