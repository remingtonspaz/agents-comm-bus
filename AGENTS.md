...
- **Codex shell environment isolation (AGE-107).** `install-codex.js` adds `"AGENTS_COMM_BUS_*" = "exclude"` under the project `[shell_environment_policy.filters]` table. Hooks retain the Codex process environment and MCP keeps its explicit `env_vars`; shell-tool children do not inherit these daemon settings. Existing filter entries are preserved. Convert same-layer legacy `exclude`/`include_only` arrays before installing; the installer refuses them without writing config. A Codex relaunch is required, and this change does not itself lift any restriction on tests or daemon launches.
