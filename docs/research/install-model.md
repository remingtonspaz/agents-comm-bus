...
- `core-daemon/bridges/<agent>/` contains **daemon-side agent protocol handlers**.
...
`core-daemon/` here means **daemon runtime**, not "agent-agnostic kernel." Agent-
specific code belongs in `core-daemon/bridges/` when it runs inside the daemon.
