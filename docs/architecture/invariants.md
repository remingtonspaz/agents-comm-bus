...
3. **Deterministic routing precedence.** *(TBD — bindings model under re-design.)*
   Why: explicit user-intent routing must beat any heuristic fallback. The
   precise primitives (binding shape, identity, mutation flow, last-active
   relationship) are being re-thought; this invariant will be re-stated once
   that lands.
   Breakage: without a stated precedence, messages get steered to whichever
   session most recently spoke, surprising users who think they've pinned a
   chat to a session.
