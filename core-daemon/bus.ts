...
import type { CurlInboundReceipt, CurlInboundReceiptScope } from "agents-comm-bus-core/storage/storage";
...
export class MessageBus {
   ...
   */
  private async receiveInboundForCurlIdempotency(
    message: Message,
    ...
    const auditDone =
      receipt.audit_recorded_at != null ||
    ...
    receipt = (await storage.getCurlInboundReceipt(scope)) ?? receipt;
    const dispatchDone =
      receipt.query_consumed_at != null ||
  ...
  private async transcriptInboundTimestamp(
  ...
  private async auditHasInboundReceived(
    conversation_id: ConversationId,
    message: Message,
    auditProbeAt: number,
  ): Promise<boolean> {
  ...
  private async tryResolveOpenQuery(
    ...
    if (!message.text) return false;
    const open = await this.options.storage.listOpenQueriesByConversation(
      conversation.conversation_id,
    );
  ...
  private async resolveQueryForCurlRecovery(
    queryId: QueryId,
   ...
   */
  private async sendAmbiguousReplyHelper(
    conversation: Conversation,
    matched: QueryRecord[],
    message: Message,
  ): Promise<void> {
    try {
      await this.send({
        ...
        payload: {
          text:
            `⚠️ ${matched.length} prompts are open — I can't tell which one you answered. ` +
      ...
      console.error(
        `agents-comm-bus: failed to send ambiguous-reply helper: ` +
  ...
  private async targetFromSession(session: SessionId): Promise<ChatRef> {
    ...
    return {
      ...
      account: botUserId as ChatRef["account"],
    };
  }
  ...
  private async botUserIdForConversation(conversation: Conversation): Promise<string> {
    ...
    if (!registration) {
      throw new Error(
        `no account registration for conversation ${conversation.conversation_id} ` +
...
export function chatRefFromConversation(conversation: Conversation): ChatRef {
  return {
    ...
    account: (conversation.bot_user_id ?? conversation.account_label) as ChatRef["account"],
