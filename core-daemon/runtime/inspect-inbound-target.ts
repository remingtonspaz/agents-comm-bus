...
import type { AgentId, Conversation, Storage } from "agents-comm-bus-core";
...
export type InboundTargetResolution =
  | "resolved"
  | "cold"
  | "ambiguous"
  | "not_found";
 ...
 */
export function resolveSessionForConversationDetailed<
  T extends InspectableSession & Parameters<SessionOwnerLiveness>[0],
>(
  sessions: readonly T[],
  conversation: { comm: string; account_label: string },
  sessionOwnerIsLive: SessionOwnerLiveness,
): DetailedSessionResolution<T> {
 ...
 */
export async function handleInspectInboundTarget(
  ...
  return {
    resolution: "resolved" satisfies InboundTargetResolution,
    registration,
    routed_session: {
